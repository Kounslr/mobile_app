...
class CantonTagTextInput extends StatefulWidget {
  ...
  final FutureOr<List<Tag>>? suggestedTags;
...
class _CantonTagTextInputState extends State<CantonTagTextInput> {
  ...
  List<Tag> get _getTagsInField {
    List<Tag> _tags = [];
    for (var i = 0; i < _tagsStringContents!.length; i++) {
      final String stringContent = _tagsStringContents![i];
      final String stringContentWithHash = widget.tagsStyler!.showHashtag ? "#$stringContent" : stringContent;

      _tags.add(Tag(name: stringContentWithHash));
    }
    return _tags;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return TypeAheadField(
      ...
      onSuggestionSelected: (Tag suggestion) {
        final String val = suggestion.name!.trim().toLowerCase();
      ...
      textFieldConfiguration: TextFieldConfiguration(
        ...
        decoration: InputDecoration(
          contentPadding: widget.textFieldStyler!.contentPadding ?? const EdgeInsets.all(20),
